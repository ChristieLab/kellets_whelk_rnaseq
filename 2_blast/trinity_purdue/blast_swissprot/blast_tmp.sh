...
cp -R $RCAC_SCRATCH/KW/blast_swissprot/database/. $tempdir/database/ || exit 1

cp $RCAC_SCRATCH/KW/blast_swissprot/$FASTA_FILE $tempdir/ || exit 1
...
# And go!
mkdir -p $RCAC_SCRATCH/KW/blast_swissprot/${myseqID}_blastoutput || exit 1

hyper-shell cluster ${tempdir}/blastTaskfile.txt \
	-o $RCAC_SCRATCH"/KW/blast_swissprot/${myseqID}_Taskfile.output" \
        -f $RCAC_SCRATCH"/KW/blast_swissprot/${myseqID}_Taskfile.failed" \
