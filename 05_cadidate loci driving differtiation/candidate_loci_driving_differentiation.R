#+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++#
#+++ Explore cadidate loci driving differentiation between HxH and ExE KW  +++#
...
p_lfc_fst_text <- p_lfc_fst + 
  geom_text(aes(x = .125, y = 2.75, label= paste0("Non-DEG SNPs: Slope = ", round(coef(lm(log2FoldChange ~ fst, data = df))["fst"], 2),", p < 0.00001 ")), hjust=0) + 
...
## test different models to normalize the residual 
library(nlme)
df$fst_fix <- df$fst+0.5

M.lm <- gls(log2FoldChange ~ fst_fix,data=df)

vf2 <- varPower(form =~ fst_fix) # Power of the variance covariate
M.gls2 <- gls(log2FoldChange ~ fst_fix ,
              weights = vf2,data=df)

vf3 <- varExp(form =~ fst_fix)
M.gls3 <- gls(log2FoldChange ~ fst_fix ,
              weights = vf3, data = df)

vf4 <- varConstPower(form =~ fst_fix)
M.gls4 <- gls(log2FoldChange ~ fst_fix ,
              weights = vf4, data = df)

res <- anova(M.lm,M.gls2,M.gls3,M.gls4,
             M.gls5,M.gls6)
...
### how about DEGs? 
deg_df$fst_fix <- deg_df$fst+0.5

deg.lm <- gls(log2FoldChange ~ fst_fix,data=deg_df)

vf2 <- varPower(form =~ fst_fix)
deg.gls2 <- gls(log2FoldChange ~ fst_fix ,
                weights = vf2,data=deg_df)
vf3 <- varExp(form =~ fst_fix)
deg.gls3 <- gls(log2FoldChange ~ fst_fix ,
                weights = vf3, data = deg_df)
vf4 <- varConstPower(form =~ fst_fix)
deg.gls4 <- gls(log2FoldChange ~ fst_fix ,
                weights = vf4, data = deg_df)
res <- anova(deg.lm,deg.gls2,deg.gls3,deg.gls4)

min(res$AIC)
anova(deg.lm, deg.gls2)
plot(resid(deg.gls2))
