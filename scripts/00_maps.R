...
### Create Map with population assignment results 
...
study_map <- ggplot(data = world) +
  geom_sf() + 
  coord_sf(xlim=c(-123.99, -116.01), ylim=c(38,31.25), expand=FALSE) + 
  annotation_scale(location = "br") +
  annotation_north_arrow(location = "bl", which_north = "true", style = north_arrow_fancy_orienteering) +
  #geom_label(data= coordinates, aes(lon,lat, label=location), nudge_x = .5, hjust=0) +
...
pdf(width = 7, height = 7, file = "~/Downloads/map.pdf")
study_map
dev.off()
