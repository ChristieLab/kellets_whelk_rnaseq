...
### written by Andy Lee, contact at andymuanlee@gmail.com, all rights reserved 
...
traitData <- readRDS("/Users/andy/KW/4_DESeq2/annotated_rnaspades/mon_nap_trait.rds")
dds <- readRDS("dds.rds")
...
traitrows <- match(sampnames, traitData$sample_ID)
datTraits <- traitData[traitrows, 3, drop=FALSE]
...
net <- readRDS("wgcna_rds/wgcna_net.rds")
...
unique(OUT$module)
OUT <- readRDS("wgcna_rds/wgcna_output.rds")

red.out <- OUT[OUT$module == "red",]
thistle.out <- OUT[OUT$module == "thistle",]
n_distinct(red.out$gene.id)
#+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++#
#+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++#
#+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++#
...
### trying to write a loop or function that exports all interesting modules: # ==============
...
geneID2GO <- readMappings(file = "/Users/andy/KW/07_geneontology/eggnog_results/all_ref_genes_to_go.txt")
geneNames <- names(geneID2GO)

# Can change this function to get MF, CC terms instead 
get_module_topGO = function(x, num_topNodes = 20 ){
  wgcna.OUT <- OUT[OUT$module==paste0(x), ]
  InterestingGenes <- wgcna.OUT$genename
  geneList <- factor(as.integer(ref.geneNames %in% InterestingGenes))
  names(geneList) <- geneNames
  GOdata <- new("topGOdata", 
                description = paste0(x," module to ref"),
                ontology = "BP",
                allGenes = geneList, 
                annot = annFUN.gene2GO, 
                gene2GO = geneID2GO)
  resultweight01 <- runTest(GOdata, algorithm = "weight01", statistic = "fisher")
  res.full <- GenTable(GOdata, resultweight01, topNodes = numSigGenes(GOdata))
  res.top <- GenTable(GOdata, resultweight01, topNodes = num_topNodes)
  write.csv(res.full, paste0("enriched_go_bp/", x, "_module_all_enrichedterms.csv"))
  write.csv(res.top, paste0("enriched_go_bp/", x, "_module_top", num_topNodes, "_enrichedterms.csv"))
} #BP
get_module_topGO = function(x, num_topNodes = 20 ){
  wgcna.OUT <- OUT[OUT$module==paste0(x), ]
  InterestingGenes <- wgcna.OUT$genename
  ...
  resultweight01 <- runTest(GOdata, algorithm = "weight01", statistic = "fisher")
  res.full <- GenTable(GOdata, resultweight01, topNodes = numSigGenes(GOdata))
  res.full$weight01Fisher <-as.numeric(res.full$weight01Fisher)
  res.full <- res.full[res.full$weight01Fisher < 0.001, ] # return significant GO terms 
  res.top <- GenTable(GOdata, resultweight01, topNodes = num_topNodes)
  write.csv(res.full, paste0("enriched_go_mf/", x, "_module_all_mf_enrichedterms.csv"))
  write.csv(res.top, paste0("enriched_go_mf/", x, "_module_top", num_topNodes, "_mf_enrichedterms.csv"))
} #MF

# Significant modules after boneferri correction 
get_module_topGO("red")
get_module_topGO("thistle")
# Potentially interesting modules 
#   get_module_topGO("darkorange")
#   get_module_topGO("floralwhite")
#   get_module_topGO("darkviolet")
#   get_module_topGO("palevioletred3")
#   get_module_topGO("brown2")
#   get_module_topGO("lightcyan")
#   get_module_topGO("yellowgreen")
#   get_module_topGO("darkgreen")
#   get_module_topGO("green")
#   get_module_topGO("grey") 
#   get_module_topGO("darkorange2")
#   get_module_topGO("purple")
#   get_module_topGO("darkmagenta")
#   get_module_topGO("cyan")
#   get_module_topGO("plum1")
#   get_module_topGO("grey60")
#   get_module_topGO("darkslateblue")
#   get_module_topGO("darkgrey")
#   get_module_topGO("darkseagreen4")