...
library("ggplot2")
library("ggtree")
library("fastreeR") #BiocManager::install("fastreeR")
library(tidyverse)
library(gridExtra)
library(dplyr)
library(adegenet)
...
all_nocross <- calc_pairwise_fst(all_nocross, facets="site")
all_nocross <- calc_pi(all_nocross, facets="site")
get.snpR.stats(all_nocross, "site", "pi")


deg_nocross <- calc_pairwise_fst(deg_nocross, facets="site")
get.snpR.stats(deg_nocross, "site", "fst")
## MON and NAP only 
...
mon <- filter_snps(all_vcf[site="MON"], maf = 0.05, maf_facets = "site")
nap <- filter_snps(all_vcf[site="NAP"], maf = 0.05, maf_facets = "site")
pol <- filter_snps(all_vcf[site="POL"], maf = 0.05, maf_facets = "site")
...
keep1 <- unique(fsts$ID[which(fsts$fst <= 0.01)])
keep1 <- which(snp.meta(all_nocross)$ID %in% keep1)
lowfst1 <- all_nocross[keep1,]
# nsnps(lowfst1)/nsnps(all_nocross) # 0.8256559
...
p_pca_usat <- ggplot(pca.x$li)  +
  ...
  scale_color_manual(values=c("#000004FF", "#D1426FFF","#FEB77EFF")) + 
...
p_str_low_fst <- structure_plots$low_fst +
  theme(legend.position = "none",
        axis.title.x = element_blank(),
...
all_tree <- ggtree(allsnptree, layout="daylight") %<+% sample_meta + 
  geom_tippoint(mapping=aes(color=site), size=2, show.legend=FALSE) +
  scale_color_manual(values=c(MON = "#000004FF", NAP = "#D1426FFF", POL="#FEB77EFF"))
...
p_low_fst_tree <- ggtree(low_fst_tree, layout="daylight") %<+% sample_meta + 
  geom_tippoint(mapping=aes(color=site), size=2, show.legend=TRUE) +
  scale_color_manual(values=c(MON = "#000004FF", NAP = "#D1426FFF", POL="#FEB77EFF"))
...
### plot figure 2 ====
pca_and_structure <- cowplot::plot_grid(p_pca_usat, p_pca_all, p_pca_deg,
                                        p_str_usat, p_str_all, p_str_deg,
                                        nrow = 2, 
                                        rel_heights= c(1,1), 
                                        align = "v",
                                        axis = "lr")

trees <- cowplot::plot_grid(usat_sub_tree, all_tree, deg_tree, 
                            nrow = 1) # p_low_fst_tree


pdf("~/KW/figures/figure 2/fig2.pdf", width = 9, height = 9)
cowplot::plot_grid(pca_and_structure, trees, 
                   nrow = 2, 
                   rel_heights= c(2,1), 
                   align = "v",
                   axis = "l"
...
# low fst  tree for supplemental 
pdf("~/KW/figures/supplemental /lowfst.PDF", width = 8, height =2.5)
cowplot::plot_grid(p_pca_low,  p_str_low_fst, p_low_fst_tree,
                   nrow = 1, 
...
# napxmon on PC2/PC3 -- supplemental
deg_nopol <- filter_snps(deg_vcf[site=-"POL"], maf = 0.05, maf_facets = "site")

deg_nopol_pca <- plot_clusters(deg_nopol, facets="site", alt.palette = c("#000004FF", "#D1426FFF", "#972C80FF"))

cowplot::plot_grid(
  deg_nopol_pca$plots$pca,
  ggplot() + 
    geom_point(aes(deg_nopol_pca$data$pca$PC2, deg_nopol_pca$data$pca$PC3, color=deg_nopol_pca$data$pca$site)) + 
    scale_color_manual(values=c("#000004FF", "#D1426FFF", "#972C80FF"), name="site") + 
    xlab("PC2") + ylab("PC3") +
    theme_bw(), 
  labels=c('A','B'), 
  align="w"
...
ggplot() + 
  geom_point(data=df, aes(x=fst, y=log2FoldChange), size = 0.3, col="darkblue") + 
  geom_smooth(data=df, method=lm, aes(x=fst, y=log2FoldChange), col="darkblue") +
