...
## retain 0.5 tpm in 25% of sample 
myCPM <- cpm(all.gene.counts)
thresh <- myCPM > 0.5
keep <- rowSums(thresh) >= .25*70 # we would like to keep genes that have at least .25*N TRUES in each row of thresh

filt.gene.counts <- all.gene.counts[keep, ] 
...
gene.counts.MON.NAP <- filt.gene.counts[, c(MON, NAP)]
...
#a ll(rownames(col.data.MON.POL) %in% colnames(gene.counts.MON.POL))
...
resSig.MON.NAP     <- res.MON.NAP[which(res.MON.NAP$padj < 0.05), ]       ## put all genes w/ padj < 0.05 in resSig
...
write.csv(as.data.frame(resSig.MON.NAP), file="tpmfilt_degs_padj05.MON.NAP.csv") 
...
####### get readnames for piccard FilterSamReads prior to calling SNPs 
