...
setwd("/Users/andy/KW/3_deg_pipeline/rnaspades_calpoly")
library('DESeq2')
library("rtracklayer")
library("ggplot2")
library("gplots")
...
gene.counts <- read.table("calpoly_featureCount_counts.txt", sep="\t", header=T, row.names=1, check.names=F)
...
samples <- read.csv("kw_sample_info.csv")
samples
#---------------- Sample processing ------------------------------------------------------------------------------------#
...
crosses <- c(m1, m2)
samples[crosses, ] # should all be sites we want to exclude (for now)!
...
col.data <- read.csv("kw_sample_no_POL_NAPXMON.csv", row.names = 1)
...
#dds <- DESeqDataSetFromMatrix(countData = gene.counts,
#                              colData = col.data,
#                              design = ~ as.factor(age) + as.factor(site))


dds$treatment <- relevel(dds$site, "NAP") ## set the untreated group as the control
dds
...
plot.treat.data <- plotPCA(rld, intgroup = c("treatment"), returnData=TRUE, n=1500) ## uses n most variable genes -- not necessarily sig DEGs!
...
write.csv(as.data.frame(resSig), file="p_05_degs_rnaspades.csv")     ## write CSV with all DEGs, padj < 0.05
#-----------------------------------------------------------------------------------------------------------------------#
